  ...
  if (match) {
    try {
      ...
      function extractVideos(obj, depth = 0) {
        ...
        // 🔥 depth limit (prevents stack/CPU explosion)
        if (depth > 8) return;
        ...
        if (Array.isArray(obj)) {
          for (const item of obj) {
            extractVideos(item, depth + 1);
          }
          return;
        }

        // 🔥 MAIN DETECTION (unchanged)
        if (
          obj.pageURL &&
          obj.title &&
          ...
          // 🔥 EARLY STOP (BIG SPEED WIN)
          if (videos.length >= 500) return;
        }

        // 🔥 traverse deeper (same logic, just safer)
